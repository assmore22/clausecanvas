...
import { Banner, Empty, RedactedLines, Hex, ExtLink, StatusChip } from "@/components/ui";
import { useTx } from "@/components/Tx";
import { useLoader } from "@/lib/hooks";
import { getOpenChallenges, getOpenAppeals, hasContract } from "@/lib/clausecanvas";
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <h1 className="mt-1 serif text-2xl font-semibold tracking-tight">Challenges &amp; appeals</h1>
        <p className="mt-1 max-w-2xl text-sm text-muted">Open challenges and appeals are re-adjudicated by the AI reviewer against the clause rubric and fresh evidence.</p>
      </div>
      ...
      {tab === "challenges" ? (
        challenges.loading && !challenges.data ? <RedactedLines rows={4} /> :
        ...
        (challenges.data?.length ?? 0) === 0 ? <Empty icon={faGavel} title="No open challenges" hint="Resolved challenges are recorded in each clause set’s audit trail." /> :
        <div className="space-y-3">
          {challenges.data!.map((c) => (
            <div key={c.challengeId} className="doc space-y-2 p-4">
              <div className="flex items-center justify-between"><span className="text-sm font-semibold">Challenge #{c.challengeId} <span className="font-normal text-muted">· clause set #{c.clauseSetId} · review #{c.reviewId}</span></span><StatusChip status={c.status} kind="decision" /></div>
              ...
              <div className="flex items-center justify-between border-t border-line pt-2 text-xs text-muted">
                ...
                <div className="flex items-center gap-2"><Link href={`/clause/${c.clauseSetId}`} className="text-accent hover:underline">clause #{c.clauseSetId} <FontAwesomeIcon icon={faArrowRight} className="h-2.5 w-2.5" /></Link><button className="btn btn-primary btn-xs" disabled={busy} onClick={() => resolveC(c.challengeId)}><FontAwesomeIcon icon={faBolt} className="h-3 w-3" /> Resolve</button></div>
              </div>
            </div>
          ))}
        </div>
      ) : (
        appeals.loading && !appeals.data ? <RedactedLines rows={4} /> :
        ...
        (appeals.data?.length ?? 0) === 0 ? <Empty icon={faScaleBalanced} title="No open appeals" hint="Appeals can be filed on rejected, revision, challenged or accepted reviews." /> :
        <div className="space-y-3">
          {appeals.data!.map((a) => (
            <div key={a.appealId} className="doc space-y-2 p-4">
              <div className="flex items-center justify-between"><span className="text-sm font-semibold">Appeal #{a.appealId} <span className="font-normal text-muted">· clause set #{a.clauseSetId} · review #{a.reviewId}</span></span><StatusChip status={a.status} kind="decision" /></div>
              ...
              <div className="flex items-center justify-between border-t border-line pt-2 text-xs text-muted">
                ...
                <div className="flex items-center gap-2"><Link href={`/clause/${a.clauseSetId}`} className="text-accent hover:underline">clause #{a.clauseSetId} <FontAwesomeIcon icon={faArrowRight} className="h-2.5 w-2.5" /></Link><button className="btn btn-primary btn-xs" disabled={busy} onClick={() => resolveA(a.appealId)}><FontAwesomeIcon icon={faBolt} className="h-3 w-3" /> Resolve</button></div>
              </div>
            </div>
          ))}
        </div>
